...
fn main() -> Result<std::process::ExitCode> {
    ...
    for event in merge_result.events {
        print!("{} {:?}", event.node_uuid, event.event_type);
    }
    ...
    if !args.force && !merge_result.warnings.is_empty() {
        print!("Warnings were generated by the merge operation. Not saving the database.");
        return Ok(std::process::ExitCode::FAILURE);
    }
    if args.dry_run {
        print!("Running in dry-run mode. Not saving the database.");
        return Ok(std::process::ExitCode::SUCCESS);
    }
